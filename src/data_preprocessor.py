...
def read_demographic():
    ...
    dataset = pd.read_csv('demographic.csv', sep=",", names = fields_d, header = 0, usecols = ["SEQN","GENDER","AGE","RACE"])
    return dataset
...
def process_examination():
    ...
    dataset = read_dataset('examination.csv')
    new_dataset = []
    for row in dataset:
        new_row = {}
    ...
    write_dataset(new_dataset, 'examination_processed.csv')

def read_examination():
    labels = ["SEQN", "PULSE", "BMI", "WEIGHT", "HEIGHT"]
    read_df = pd.read_csv('examination_processed.csv', sep=",", names = labels, header = 0)
    return read_df
...
def read_labs():
    ...
    dataset = pd.read_csv('labs.csv', sep=",", names = fields_l, header = 0, usecols = ["SEQN","CALCIUM","CHOLESTEROL","PROTEIN","URIC_ACID","HEMOGLOBIN"])
    return dataset